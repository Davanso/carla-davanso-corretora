...
export const propertyMetadataSchema = z.strictObject({
  ...
  iptuFee: z.number().finite().min(0).max(21_474_836.47),
  landAreaM2: z.number().int().positive("Informe a metragem do terreno.").max(1_000_000),
  builtAreaM2: z.number().int().min(0).max(1_000_000),
  bedrooms: z.number().int().min(0).max(1_000),
  suites: z.number().int().min(0).max(1_000),
  livingRooms: z.number().int().min(0).max(1_000),
  bathrooms: z.number().int().min(0).max(1_000),
  parkingSpots: z.number().int().min(0).max(1_000),
  ownerName: z.string().trim().max(160, "O nome do proprietário é muito longo."),
  ownerPhone: z.string().trim().max(40, "O telefone do proprietário é muito longo."),
  city: shortText("a cidade"),
  district: shortText("o bairro"),
  community: z.string().trim().max(160, "O nome do empreendimento é muito longo."),
  street: z.string().trim().max(180, "O endereço é muito longo."),
  addressNumber: z.string().trim().max(30, "O número é muito longo."),
  addressComplement: z.string().trim().max(120, "O complemento é muito longo."),
  zipCode: z.string().trim().max(16, "O CEP é muito longo."),
  isCondo: z.boolean(),
