...
export async function PATCH(request: Request, context: Context) {
  ...
  const merged = {
    ...
    iptuFee: values.iptuFee ?? (existing.iptuFeeCents ?? 0) / 100,
    landAreaM2: values.landAreaM2 ?? existing.landAreaM2 ?? existing.areaM2,
    builtAreaM2: values.builtAreaM2 ?? existing.builtAreaM2 ?? existing.areaM2,
    bedrooms: values.bedrooms ?? existing.bedrooms ?? 0,
    suites: values.suites ?? existing.suites ?? 0,
    livingRooms: values.livingRooms ?? existing.livingRooms ?? 0,
    ...
    const property = await prisma.$transaction(async (transaction) => {
      ...
      return transaction.property.update({
        ...
        data: {
          ...
          iptuFeeCents: merged.iptuFee ? Math.round(merged.iptuFee * 100) : null,
          areaM2: merged.landAreaM2 || merged.builtAreaM2 || existing.areaM2,
          landAreaM2: merged.landAreaM2 || null,
          builtAreaM2: merged.builtAreaM2 || null,
